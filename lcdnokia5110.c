...
void LCDNokia_init(void){
	...
	CLOCK_EnableClock(kCLOCK_Spi0);
	    ...
	    gpio_pin_config_t LCD_pin_config = {kGPIO_DigitalOutput, 0};

	    GPIO_PinInit(GPIOD, 0, &LCD_pin_config);
	    GPIO_PinInit(GPIOD, 3, &LCD_pin_config);

	    PORT_SetPinConfig(PORTD, 0, &config_port); //pin rst
	    PORT_SetPinConfig(PORTD, 1, &config_spi); //pin clk
	    PORT_SetPinConfig(PORTD, 2, &config_spi); //pin sout
	    PORT_SetPinConfig(PORTD, 3, &config_port); //pin dc

	    GPIO_ClearPinsOutput(GPIOD, 1<< 0);
	    LCD_delay();
	    GPIO_SetPinsOutput(GPIOD, 1<< 0);
...
{
	if(DataOrCmd)
		GPIO_SetPinsOutput(GPIOD,1 << 3);
	else
		GPIO_ClearPinsOutput(GPIOD,1 << 3);


	 // Prepare to send.
	isFinished = false;
	spi_master_xfer.txData = &data;
    spi_master_xfer.rxData = NULL;
    spi_master_xfer.dataSize = sizeof(uint8_t);
    spi_master_xfer.configFlags = kDSPI_MasterCtar0 | kDSPI_MasterPcs0 | kDSPI_MasterPcsContinuous;
